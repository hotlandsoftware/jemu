...
RcaDisplay *rca_display_sdl_create(int scale) {
    return rca_display_sdl_create_mono("JEMU", CDP1861_DISPLAY_W,
...
static RcaDisplay *sdl_create_common(const char *title, int w, int h,
    ...
    c->pixel_on = on;
    c->pixel_off = off;
    c->window = SDL_CreateWindow(title ? title : "JEMU", SDL_WINDOWPOS_CENTERED,
...
RcaDisplay *rca_display_create_mono(JemuDisplayType type, const char *title,
    ...
    case JEMU_DISPLAY_GTK:
        return rca_display_gtk_create_mono(title, w, h, scale, on, off);
#endif
...
RcaDisplay *rca_display_create_indexed(JemuDisplayType type, const char *title,
                                       int w, int h, int scale,
