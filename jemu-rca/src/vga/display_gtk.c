...
static RcaDisplay *gtk_create_common(const char *title, int w, int h,
    ...
    gtk_window_set_title(GTK_WINDOW(c->window), title ? title : "JEMU");
    ...
    g_signal_connect(quit_item, "activate", G_CALLBACK(on_quit_activate), c);
...
RcaDisplay *rca_display_gtk_create_mono(const char *title, int w, int h,
                                        ...
                                        uint32_t off) {
    return gtk_create_common(title, w, h, scale, NULL, 0, on, off);
}
