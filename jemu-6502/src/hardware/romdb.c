...
int mos_romdb_load_dir(MosConfig *cfg, const char *dir_path,
    ...
    if (!d) {
        fprintf(stderr, "jemu-6502: cannot open rom dir '%s'\n", dir_path);
        return -1;
    }
    ...
    while ((ent = readdir(d)) != NULL) {
        ...
        if (!jemu_sha256_file(path, digest)) {
            fprintf(stderr, "jemu-6502: cannot read '%s'\n", path);
            continue;
        }
        char hex[65];
        ...
        if (!e) {
            ...
            if (!any)
                fprintf(stderr, "jemu-6502: unknown ROM '%s' (sha256=%s)\n",
                        ent->d_name, hex);
            continue;
        }

        if (cfg->n_roms >= MOS_MAX_ROM_LOADS) {
            fprintf(stderr, "jemu-6502: too many ROMs (max %d)\n", MOS_MAX_ROM_LOADS);
            break;
        }
        ...
        cfg->n_roms++;
        loaded++;
        printf("jemu-6502: romdb matched %s (%s) → 0x%04X\n",
...
void mos_romdb_print_needed(const char *machine_alias) {
    fprintf(stderr, "jemu-6502: no ROMs provided for '%s'\n", machine_alias);
